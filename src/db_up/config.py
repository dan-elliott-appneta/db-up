...
def _load_monitor_config(file_config: Dict[str, Any]) -> MonitorConfig:
    ...
    """
    check_interval = int(os.getenv('DB_CHECK_INTERVAL', file_config.get('check_interval', 60)))
    max_retries = int(os.getenv('DB_MAX_RETRIES', file_config.get('max_retries', 3)))
    retry_backoff = os.getenv('DB_RETRY_BACKOFF') or file_config.get('retry_backoff', 'exponential')
    retry_delay = int(os.getenv('DB_RETRY_DELAY', file_config.get('retry_delay', 5)))
    retry_jitter = _parse_bool(os.getenv('DB_RETRY_JITTER'), file_config.get('retry_jitter', True))
    health_check_query = file_config.get('health_check_query', 'SELECT 1 AS health_check')
    read_only_mode = _parse_bool(os.getenv('DB_READ_ONLY_MODE'), file_config.get('read_only_mode', True))
...
def _load_logging_config(file_config: Dict[str, Any]) -> LoggingConfig:
    ...
    file_path = os.getenv('DB_LOG_FILE') or file_config.get('file_path', 'logs/db-up.log')
    max_file_size = int(os.getenv('DB_LOG_MAX_SIZE', file_config.get('max_file_size', 10485760)))
    backup_count = int(os.getenv('DB_LOG_BACKUP_COUNT', file_config.get('backup_count', 5)))
    ...
    redact_hostnames = _parse_bool(
        os.getenv('DB_LOG_REDACT_HOSTNAMES'),
        file_config.get('redact_hostnames', False)
    )
