...
class DatabaseConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.database:
            raise ValueError("Database name is required. Set DB_NAME environment variable.")
...
class MonitorConfig:
    ...
    Attributes:
        ...
        retry_backoff: Backoff strategy: fixed, linear, exponential (default: exponential)
        retry_delay: Base delay between retries in seconds (default: 5)
        retry_jitter: Add randomness to retry delays (default: True)
        health_check_query: SQL query for health check (default: SELECT 1 AS health_check)
    ...
    def __post_init__(self) -> None:
        ...
        if any(keyword in query for keyword in ["DROP", "DELETE", "UPDATE", "INSERT", "ALTER", "CREATE"]):
            raise ValueError("health_check_query cannot contain data modification statements")
...
class MetricsConfig:
    ...
    Attributes:
        ...
        host: Host to bind metrics server to (default: 0.0.0.0)
    """
